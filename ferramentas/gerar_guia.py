...
def render(text):
    ...
    while i < len(lines):
        ...
        if line.startswith('```'):
            ...
            out.append('<div class="codigo"><div class="codigo-topo"><span>' + html.escape(lang or 'texto') + '</span><button type="button" class="copiar">Copiar código</button></div><pre tabindex="0"><code>' + html.escape('\n'.join(code)) + '</code></pre></div>')
            i += 1
            continue
        if line.startswith('### '):
...
<title>Como criar MODs — SEELE</title><link rel="stylesheet" href="../fontes.css"><link rel="stylesheet" href="../tokens.css"><link rel="stylesheet" href="guia.css"></head>
<body><a class="pular" href="#conteudo">Pular para a documentação</a>
<header class="topo-guia"><a class="marca" href="../">SEELE <span>MODS</span></a><nav aria-label="Site"><a href="../">Catálogo</a><a href="./" aria-current="page">Criar um MOD</a><a href="../#/publicar">Publicar</a></nav><a class="download" href="guia-criacao-mods.md" download>Baixar guia .md ↓</a></header>
<div class="introducao"><div><p class="coordenada">Manual de criação / API 2</p><h1>Faça o SEELE<br><span>do seu jeito.</span></h1><p class="resumo">Da primeira ideia ao pacote publicado. Aprenda o caminho ou consulte o contrato exato de cada operação.</p></div><div class="ficha"><span>REFERÊNCIA DO DESENVOLVEDOR</span><dl><div><dt>Manifesto</dt><dd>Schema 1</dd></div><div><dt>API de MOD</dt><dd>Versão 2</dd></div><div><dt>Revisão</dt><dd>18 SET 2026</dd></div><div><dt>Conteúdo</dt><dd>23 capítulos · exemplo instalável</dd></div></dl><a href="exemplos/contador.zip" download>Baixar MOD de exemplo ↓</a></div></div>
...
<main id="conteudo" tabindex="-1"><section id="resultados" aria-label="Resultados da busca" hidden><h2>Resultados</h2><p id="contagem" role="status"></p><div id="lista-resultados"></div><button type="button" id="limpar-busca">Limpar busca</button></section>'''+''.join(panels)+'''</main>
