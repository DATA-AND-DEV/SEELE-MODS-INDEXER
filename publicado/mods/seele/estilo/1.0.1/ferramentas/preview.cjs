...
const server=http.createServer(async(req,res)=>{
  ...
  if(url.pathname==='/request' && req.method==='POST'){
    try{
      ...
      const q=JSON.parse(body),mod=q.mod;if(!['estilo'].includes(mod))throw Error('unknown-mod');
  ...
  const routes={'/estilo.js':'cliente/main.js','/preview.js':'ferramentas/preview.js','/preview.css':'ferramentas/preview.css'};
